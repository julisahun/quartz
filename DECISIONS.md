...
| 5 | Name and repo | **`quarts`** | `julisahun/quarts`, following `aegis` / `pirdle` / `pergamino`. |
...
- **The server serves the PWA itself** (`QUARTS_WEB_DIR`) instead of a second
...
- **`.quarts*` is a reserved prefix** in the vault: temp files during atomic
